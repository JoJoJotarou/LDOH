-- Migration: 反转 is_only_maintainer_visible 语义（幂等）
-- 目标语义：TRUE = 仅站长可见，FALSE = 大众可见
-- 注意：本迁移中的数据取反步骤仅应执行一次，请勿重复执行。

ALTER TABLE public.site
  ALTER COLUMN is_only_maintainer_visible SET DEFAULT false;

UPDATE public.site
SET is_only_maintainer_visible = NOT COALESCE(is_only_maintainer_visible, false);
